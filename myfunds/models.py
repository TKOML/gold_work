...
class Portfolio(models.Model):
  ...
  def update_investment(self):
    investment = 0
    holdings = FundHolding.objects.filter(portfolio=self)
    for c in holdings:
...
class FundHolding(models.Model):
  ...
  fund_symbol = models.CharField(default='', max_length=25)
  ...
  def save(self, *args, **kwargs):
    ...
    super(FundHolding, self).save(*args, **kwargs)

  def __str__(self):
    return str(self.portfolio) + " -> " + self.fund_symbol + " " + str(self.number_of_shares)
